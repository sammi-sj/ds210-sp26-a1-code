...
impl ChatbotV2 {
    ...
    pub fn new(model: Llama) -> ChatbotV2 {
        return ChatbotV2 {model:model.clone(), chat_session: model.chat().with_system_prompt("The assistant will act British")};
    ...
    pub async fn chat_with_user(&mut self, message: String) -> String {
        ...
        let output = self.chat_session.add_message(message).await.expect("failure");
